...
def fid_score(codes_g, codes_r, eps=1e-6):
    ...
    assert mn_g.shape == mn_r.shape, "Training and test mean vectors have different lengths"
    assert cov_g.shape == cov_r.shape, "Training and test covariances have different dimensions"
    ...
    if not np.isfinite(covmean).all():
        ...
        covmean = linalg.sqrtm((cov_g + offset).dot(cov_r + offset))
    ...
    return diff.dot(diff) + np.trace(cov_g) + np.trace(cov_r) - 2 * tr_covmean
...
def evaluate_fid_score(fake_images, real_images, norm=True):
    ...
    if norm:
        fake_images = preprocess_fake_images(fake_images, norm=norm)
